	...
	public static ArrayList<double[]> motionToData(ArrayList<Motion> mList, Motion first, boolean mirror){
		...
		for (int i = 0; i < pmList.size(); i++) {
			...
			if (useOrientation){
				...
				data = MathUtil.concatenate(data, getOrientationData(OrientationJointList, mList.get(i)));
			} else {
				data = getPositionData(KeyJointList, pm.pointData);
			}
...
//		return new boolean[]{ true, true };
	...
	public static double[] getOrientationData(String[] jointList, Motion motion){
		double[] data = new double[jointList.length*3];
		...
		for (int i = 0; i < jointList.length; i++) {
			...
			if (v.length() > Math.PI - 0.05 && !jointList[i].equals("LeftHand")){
				AxisAngle4d a = new AxisAngle4d();
				a.set(matrix);
				System.out.println(motion.motionData.file.getName() + " : " + motion.frameIndex + " : " + jointList[i] + "\t\t" + v.length() + " : " + v);
//				throw new RuntimeException();
			}
			data[i*3 + 0] = v.x;
			data[i*3 + 1] = v.y;
...
//	}
	public static void setAllJoints(){
		...
		jointPairs = new String[][]{
				{ "Hips", "LeftUpLeg", "LeftLeg", "LeftFoot", "LeftToe", "LeftToe_End" },
				{ "Neck", "LeftArm", "LeftForeArm", "LeftHand", "LeftHand_End" },
				{ "Hips", "RightUpLeg", "RightLeg", "RightFoot", "RightToe", "RightToe_End" },
