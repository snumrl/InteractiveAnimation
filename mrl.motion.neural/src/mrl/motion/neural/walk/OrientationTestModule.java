...
public class OrientationTestModule extends Module{
	...
	private ArrayList<Motion> totalMotion = new ArrayList<Motion>();
...
//		String folder = "walk_zero3";
		...
		addMenu("&Menu", "Test &Stop\tCtrl+S", SWT.MOD1 + 'S', new Runnable() {
			...
			public void run() {
				...
				getModule(ItemListModule.class).addSingleItem("TotalMotion2", new MotionData(totalMotion));
...
//					double mv = normal.xMeanAndStd[0][2] + movement*normal.xMeanAndStd[1][2];
					...
					Motion motion = g.updateByOri(output);
					mData = new MotionData(Utils.singleList(motion));
//					mainViewer.addCameraTracking(g.pose.position3d());
	...
	public static void main(String[] args) {
		MainApplication.run(new OrientationTestModule());
		OgreJNI.close();
	}
}
