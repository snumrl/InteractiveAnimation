...
public class SelectiveDirectionTestModule extends Module{
	...
	private boolean alwaysUseDirection = false;
...
//					double mv = normal.xMeanAndStd[0][2] + movement*normal.xMeanAndStd[1][2];
					...
					double[] x;
					if (alwaysUseDirection){
						x = new double[]{ v.x, v.y, d.x, d.y};
					} else {
						x = new double[]{ v.x, v.y, d.x, d.y, useDirection ? 1 : 0 };
					}
					x = normal.normalizeX(x);
					if (!alwaysUseDirection && !useDirection){
						x[2] = x[3] = 0;
					}
...
//					mainViewer.addCameraTracking(g.pose.position3d());
					...
					if (useDirection){
						getModule(ItemListModule.class).addSingleItem("Direction", new Pose2d(g.pose.position, direction), new ItemDescription(new Vector3d(0, 1, 0)));
					} else {
	...
	public static void main(String[] args) {
		MainApplication.run(new SelectiveDirectionTestModule());
		OgreJNI.close();
	}
}
