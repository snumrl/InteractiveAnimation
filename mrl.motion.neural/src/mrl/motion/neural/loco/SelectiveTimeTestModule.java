...
public class SelectiveTimeTestModule extends Module{
...
//		String folder = "walk_zero3";
		...
		addMenu("&Menu", "Test 1\tCtrl+1", SWT.MOD1 + '1', new Runnable() {
			...
			public void run() {
				selectiveTime -= 5;
				selectiveTime = Math.max(selectiveTime, 15);
				System.out.println("stime : " + selectiveTime);
			}
		});
		addMenu("&Menu", "Test 2\tCtrl+2", SWT.MOD1 + '2', new Runnable() {
			...
			public void run() {
				selectiveTime += 5;
				selectiveTime = Math.min(selectiveTime, 75);
				System.out.println("stime : " + selectiveTime);
			}
		});
	}
...
//					double mv = normal.xMeanAndStd[0][2] + movement*normal.xMeanAndStd[1][2];
					...
					double[] x = new double[]{ v.x, v.y, selectiveTime };
...
//					mainViewer.addCameraTracking(g.pose.position3d());
	...
	public static void main(String[] args) {
		MainApplication.run(new SelectiveTimeTestModule());
		OgreJNI.close();
	}
}
