...
public class CMUMayaSkeletonGenerator {
	...
	public static void main(String[] args) throws IOException {
		...
		double yHeight = MathUtil.getTranslation(motion.root()).y;
		for (Pair<String, Matrix4d> entry : motion.entrySet()){
			entry.second.setIdentity();
		}
		motion.root().setTranslation(new Vector3d(0, yHeight, 0));
		...
		for (Joint j : mData.skeletonData.values()){
			...
			if (j.parent == null || j.transition.length() < 0.00001) continue;
