...
public class BallTrajectoryGenerator {
	...
	public static String BALL_ANN_FOLDER = "ballAnnotation";
...
//	private static double BALL_RADIUS = 24.64/2;
	...
	private static Point3d getBallPositionByHand(HashMap<String, Matrix4d> transform, String jointName){
		...
		Matrix4d t = transform.get(jointName);
...
//			System.out.println("pp : " + i + " : " + Utils.toString(p, startPos, endPos, v_hori, t));
	...
	private static MPLink getLink(MotionParamGraph graph, int m1, int m2){
		for (MPLink link : graph.linkList()){
			if (link.source().motionIndex == m1 && link.target().motionIndex == m2) return link;
		}
		return null;
	}
...
//		ArrayList<MotionAnnotation> annList = eventAnn.getTotalAnnotations();
//		int margin = 20;
//		MotionAnnotation ann = annList.get(16);
////		int m1 = database.findMotion("s_003_1_1.bvh", 172).motionIndex;
////		int m2 = database.findMotion("s_003_1_1.bvh", 409).motionIndex;
//		int m1 = database.findMotion(ann.file, ann.startFrame - margin).motionIndex;
//		int m2 = database.findMotion(ann.file, ann.endFrame + margin).motionIndex;
//		Motion[] mList = database.getMotionList();
//		ArrayList<Motion> motionList = new ArrayList<Motion>();
//		for (int i = m1; i <= m2; i++) {
//			motionList.add(mList[i]);
//		}
		...
		param.loadGraph("dribbleGraph.graph");
		
		@SuppressWarnings("unchecked")
		ArrayList<int[]> segments = (ArrayList<int[]>)FileUtil.readObject("dribble.segment");
		Motion[] mList = database.getMotionList();
		int count = 0;
		for (int i = 0; i < segments.size()-1; i++) {
			int m1 = segments.get(i)[1];
			int m2 = segments.get(i+1)[0];
			count += m1 - segments.get(i)[0] + 1;
			System.out.println(count + " : " + mList[m1].ballContact + ", " + mList[m1+1].ballContact + " , " + mList[m2].ballContact + " :: " + m1 + " , " + m2);
			MPLink link = getLink(param, m1, m2);
			System.out.println(link.isSequential + " : " + link.distance() + " :: " + param.dist(m1, m2));
		}
...
//		tEdit.timeOffset = 0.35;
		...
		ArrayList<Motion> mmList = MotionData.divideByKnot(segment.getEntireMotion());
		segment = new MotionSegment(Utils.toArray(mmList), MotionSegment.BLEND_MARGIN(), mmList.size() - MotionSegment.BLEND_MARGIN() - 1, true);
		
		ArrayList<Motion> motionList = segment.getMotionList();
		
		TimeBasedList<Point3d> ballTrajectory = new TimeBasedList<Point3d>();
		ballTrajectory.addAll(new BallTrajectoryGenerator().generate(motionList));
		System.out.println("traj size :: " + ballTrajectory.size());
		MainViewerModule.runWithDescription(new Object[]{
				new MotionData(motionList), ballTrajectory
		}, new ItemDescription[]{
				null, new ItemDescription(BALL_RADIUS)
		});
	}
}
