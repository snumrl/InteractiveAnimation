...
public class TrajectoryEditParam {
	...
	public TrajectoryEditParam(){
		this(0.3, 45, 0.2);
	}
	
	public TrajectoryEditParam(double lengthOffset, double angleOffset, double timeOffset) {
...
//				int div = 15;
	...
	public MotionSegment editUnitSegment(MotionSegment segment, int iter){
		...
		time *= 1 + rTime;
		return MotionEditParam.getEditedSegment(segment, 0, segment.length()-1, pose, (int)Math.round(time));
	}
}
