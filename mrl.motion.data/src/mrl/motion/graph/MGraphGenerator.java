...
//		if (source.isAir || target.isAir) return Integer.MAX_VALUE;
	...
	public double getDiff(MGGNode source, MGGNode target){
		return dist.getDistance(source.motion.motionIndex, target.motion.motionIndex);
		...
		Utils.runMultiThread(new IterativeRunnable() {
			...
			public void run(int index) {
				...
				for (int j = 0; j < edges.length; j++) {
					...
					double sAlignAngle = -MotionTransform.alignAngle(source.motion.root());
					...
					Vector3d v1 = MathUtil.getTranslation(source.motion.root());
