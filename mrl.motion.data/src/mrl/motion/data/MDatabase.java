...
//			MotionEditJNI.instance.checkFootContact(motionData);
	...
	private void loadAnnotations(){
		...
		eventAnn = new MotionAnnotationManager(Configuration.ANNOTATION_FOLDER);
...
//		}
		...
		for (int i = 0; i < typeList.length; i++) {
			if (typeList[i] != null){
				...
				if (info.frameIndex < Configuration.BLEND_MARGIN + 2
						|| info.motionData.motionList.size() - info.frameIndex - 1 < Configuration.BLEND_MARGIN + 2){
					typeList[i] = null;
				}
			}
		...
		while (true){
			...
			for (int i = 0; i < motionList.size()-2; i++) {
				...
				if (t1.equals(t2) || t1.equals(t3)){
					...
					switch (i%3) {
					case 0: t1.x += 0.00013; break;
					case 1: t1.x -= 0.00011; break;
					case 2: t1.z += 0.00012; break;
					}
